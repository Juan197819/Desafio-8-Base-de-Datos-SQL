...
let tablaProductosSQLite, tablaProductosSQL;
...
knexSQL.schema.dropTableIfExists('Productos')
    ...
    .finally(()=>{
        tablaProductosSQL = knexSQL.schema.createTable('Productos', table=>{
            table.increments('id')
            table.string('title')
            table.float('price')
...
knexSQL.from('Productos').select('*')
    .then((result) => {
        ...
        for (const  fila of result) {
            console.log(`TITULO: ${fila['title']}, PRECIO: ${fila.price}`)
        }
    }).catch((err) => {
...
knexSQLite.schema.dropTableIfExists('Mensajes')
    ...
    .finally(()=>{
         tablaProductosSQLite = knexSQLite.schema.createTable('Mensajes', table=>{
            table.increments('id')
            table.string('mail')
